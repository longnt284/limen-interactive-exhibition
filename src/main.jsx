...
// h,s,l,alpha của chữ nền cho mỗi giao diện. Độ lệch hue/lum của tám chương lấy từ bảng TINT trong diagram.js,
// dùng chung với nền CSS và bảng trạng thái của cảnh 3D.
const DISPLAY={dark:[118,10,49,.17],light:[105,15,47,.15]};
// Lớp 3D chỉ còn là khí quyển nền. Nếu nó hỏng, sơ đồ và nội dung vẫn kể trọn câu chuyện nên không cần thông báo.
class Boundary extends Component{state={error:false};static getDerivedStateFromError(){return{error:true}}render(){return this.state.error?null:this.props.children}}
function App(){
 ...
 useEffect(()=>{document.documentElement.dataset.motion=reduced?'calm':'full'},[reduced]);
 ...
 return <><a className="skip" href="#threshold">Đến hành trình</a>
  <div className="world" aria-hidden="true"><Boundary><Suspense fallback={null}><Scene signal={signal} pointer={pointer} paused={paused} reduced={reduced} speed={speed} hidden={hidden} light={light} ambient/></Suspense></Boundary></div>
  <Diagram signal={signal} reduced={reduced} paused={paused} light={light} hidden={hidden} speed={speed}/>
  ...
  <dialog ref={dialog} aria-labelledby="about-title" onCancel={()=>setInfo(false)} onClose={()=>setInfo(false)} onClick={e=>{if(e.target===dialog.current)setInfo(false)}}><button className="close icon-button" aria-label="Đóng" onClick={()=>setInfo(false)}><X/></button><span className="eyebrow">VỀ TRIỂN LÃM</span><h2 id="about-title">Giữa hai thế giới.</h2><p>LIMEN là một triển lãm nghệ thuật số phi thương mại. Tám chương nối tiếp nhau, từ hình hài đầu tiên đến phân rã và tái sinh.</p><p>Cuộn để khám phá, hoặc dùng phím mũi tên lên xuống, Home và End để đi giữa các chương. Thanh điều hướng bên cạnh đưa thẳng tới một chương bất kỳ.</p><p>Mỗi chương là một bố cục khác của cùng một sơ đồ. Mười sáu nút và các đường nối giữ nguyên danh tính từ đầu tới cuối; khi cuộn, chúng dịch chuyển, nối lại và tự vẽ thành hình thái của chương kế tiếp.</p><p>Âm thanh tắt mặc định. Khi thiết bị yêu cầu giảm chuyển động, nội dung hiển thị trực tiếp và sơ đồ đứng yên ở bố cục của từng chương.</p><label className="motion-option"><input type="checkbox" checked={reduced} onChange={e=>setReduced(e.target.checked)}/> Giảm chuyển động</label><button className="text-link" onClick={()=>setInfo(false)}>Trở lại hành trình <ArrowRight/></button></dialog>
 </>
}
